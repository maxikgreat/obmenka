...
import styles from './CrossRate.module.scss';
import {Rate1, Rate2, Rate3} from '../../assets';
import {useRateActions} from '../../state/hooks/UseActions';
import {useSelector} from '../../state/hooks';
import RateEntity from 'entities/Rate';
import RateEdit from '../orders/details/RateEdit';

const CrossRate: React.FC = () => {
  ...
  const setRates = () => {
    if (rateList.isSuccess) {
      const rozRate = rateList.rates.filter((r) => r.isCross);
      setRozRate(rozRate);
    }
  };
  ...
  const rates = [
    {
      id: rozRate.length > 0 && rozRate.filter((r) => r.name === 'EUR/USD')[0].id,
      name: rozRate.length > 0 && rozRate.filter((r) => r.name === 'EUR/USD')[0].name,
      buy: rozRate.length > 0 && rozRate.filter((r) => r.name === 'EUR/USD')[0].buy,
      sell: rozRate.length > 0 && rozRate.filter((r) => r.name === 'EUR/USD')[0].sell,
      image: Rate1,
    },
    {
      id: rozRate.length > 0 && rozRate.filter((r) => r.name === 'USD/RUB')[0].id,
      name: rozRate.length > 0 && rozRate.filter((r) => r.name === 'USD/RUB')[0].name,
      buy: rozRate.length > 0 && rozRate.filter((r) => r.name === 'USD/RUB')[0].buy,
      sell: rozRate.length > 0 && rozRate.filter((r) => r.name === 'USD/RUB')[0].sell,
      image: Rate2,
    },
    {
      id: rozRate.length > 0 && rozRate.filter((r) => r.name === 'GBR/USD')[0].id,
      name: rozRate.length > 0 && rozRate.filter((r) => r.name === 'GBR/USD')[0].name,
      buy: rozRate.length > 0 && rozRate.filter((r) => r.name === 'GBR/USD')[0].buy,
      sell: rozRate.length > 0 && rozRate.filter((r) => r.name === 'GBR/USD')[0].sell,
      image: Rate3,
    },
  ];
