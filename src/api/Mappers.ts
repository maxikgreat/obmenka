...
export const mapUpdateRateRequestToGQL = (
  updateRateRequest: UpdateRateRequest,
): UpdateRateRequest => ({
  id: updateRateRequest.id,
  buy: updateRateRequest.buy,
  sell: updateRateRequest.sell,
});
...
export const mapRateFromGQL = (rate: GQLRate): Rate => ({
  id: rate.id,
  name: rate.name,
  buy: rate.buy,
  sell: rate.sell,
...
export const mapRatesFromGQL = (rates: GQLRate[]): Rate[] =>
  rates.map((cuisine) => mapRateFromGQL(cuisine));
