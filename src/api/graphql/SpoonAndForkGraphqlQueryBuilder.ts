...
export const mutationUpdateRate = createMutationWithVariables<
  MutationUpdateRateArgs,
  {updateRate: boolean},
  void
>(
  gql`
    mutation updateRate($id: ID!, $buy: Float!, $sell: Float!) {
      updateRate(id: $id, buy: $buy, sell: $sell)
    }
  `,
  () => undefined,
);
...
export const ratesQuery = createQuery<{rates: Rate[]}, Rate[]>(
  gql`
    ${RateFragment()}
    query {
      rates {
